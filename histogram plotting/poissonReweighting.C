...
double poissonCalculate(int simE, const char *variance) {
    ...
    float poisson = (exp(- var)*pow(var, simE))/(factorial(simE));
    return poisson;
}


void poissonReweighting(const char *process, const char *variance) // mult is the simulated multiplicity, track is the trk being measured - energy threshold is based off of the trk
{
    gStyle->SetOptStat(0);
    ...
    // initializations
    TFile* infile1e = TFile::Open(Form("%s1e_allruns_trk.root", process));
    ...
    for (int trk = 0; trk <= 5; trk++) {
        for (unsigned int layer = 0; layer < layerNum.size(); layer++) {
            ...
            for (int simE = 2; simE <= 4; simE++) {
                TFile* infiletemp = TFile::Open(Form("%s%ie_allruns_trk.root", process, simE));
    ...
    TFile *outfile = new TFile(Form("%s_allruns_%spoisson.root", process, variance), "RECREATE");
